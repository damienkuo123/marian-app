(() => {
  ...
  const VERSION = 'M2.5B4R3A3-PRODUCTION';
  ...
  // M2.5B4R3A2: the wardrobe uses one native vertical scroll page.
  // Keep the historical compatibility function as a harmless scroll reset
  // because older inline Dashboard code may still call initD322Drawer().
  function initScrollWardrobe(reset = false) {
    const page = document.querySelector(
      '#dressing-room-overlay .dressing-room-page'
    );
    if (reset && page) page.scrollTop = 0;
  ...
  window.initD322Drawer = initScrollWardrobe;
