(() => {
  ...
  const rewardZoneEnabled = params.get('rewardZone') !== '0';
  ...
  async function prepareUnityWorld() {
    ...
    unityInitializePromise = (async () => {
      ...
      gate.worldPrepared = true;
      await window.TappieChallengeState?.transition?.('READY_GATE');
      setGateProgress(1);
      updateGate();
    })().catch(error => {
  ...
  async function getUnityProductionDiagnostics(label, timeoutMs = 3500) {
    const pending = unityArena.waitForRuntime('diagnostics', null, timeoutMs);
    await unityArena.send('GetDiagnostics', label || 'INTRO_LIVE_READY_PROBE');
    const detail = await pending;
    const d = detail?.diagnostics || {};
    const snapshot = {
      label,
      ready: Boolean(d.ready),
      bootStage: String(d.bootStage || ''),
      lastErrorCode: String(d.lastErrorCode || ''),
      lastError: String(d.lastError || ''),
      playerReady: Boolean(d.player?.ready),
      opponentReady: Boolean(d.opponent?.ready),
      playerRenderers: Number(d.player?.rendererCount || 0),
      opponentRenderers: Number(d.opponent?.rendererCount || 0),
      introActive: Boolean(d.introActive),
      introPlayed: Boolean(d.introPlayed),
      introState: String(d.introState || ''),
      cameraState: String(d.cameraState || '')
    };
    window.__TAPPIE_INTRO_LIVE_READY_DIAGNOSTICS__ = {
      ...(window.__TAPPIE_INTRO_LIVE_READY_DIAGNOSTICS__ || {}),
      [label]: snapshot
    };
    return snapshot;
  ...
  function compactIntroDiag(d) {
    if (!d) return 'diagnostics unavailable';
    return `frontend=${d.frontendReadyCache ?? '-'} liveReady=${d.ready} boot=${d.bootStage || '-'} error=${d.lastErrorCode || '-'} player=${d.playerReady}/${d.playerRenderers} opponent=${d.opponentReady}/${d.opponentRenderers} intro=${d.introState || '-'} camera=${d.cameraState || '-'}`;
  }

  function waitForIntroDispatchOutcome(timeoutMs = 3500) {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        window.removeEventListener('tappie:challenge-runtime', listener);
        reject(new Error('Unity Intro dispatch 3.5 秒無 intro-start / intro-skipped / runtime-not-ready status'));
      }, timeoutMs);
      const listener = event => {
        const detail = event.detail;
        const kind = String(detail?.kind || detail?.type || detail?.event || '').toLowerCase();
        const message = String(detail?.message || '');
        if (kind === 'intro-start' || kind === 'intro-skipped' || kind === 'error' ||
            (kind === 'status' && /intro|runtime is ready|runtime-not-ready/i.test(message))) {
          clearTimeout(timer);
          window.removeEventListener('tappie:challenge-runtime', listener);
          resolve({ kind, message, detail });
        }
      };
  ...
  async function playMatchIntroWithLiveReadyProbe() {
    if (!unityArena) throw new Error('TappieChallengeArena bridge missing');
    const frontendReadyBefore = Boolean(unityArena?.isReady?.());
    let beforeReset;
    try {
      beforeReset = await getUnityProductionDiagnostics('beforeReset');
    } catch (probeError) {
      throw new Error(`Frontend ready cache=${frontendReadyBefore}; Unity GetDiagnostics failed：${probeError?.message || probeError}`);
    }
    beforeReset.frontendReadyCache = frontendReadyBefore;
    if (!beforeReset.ready) {
      throw new Error(`Frontend ready cache=${frontendReadyBefore}; Unity LIVE runtime 在 Intro 前不是 ready：${compactIntroDiag(beforeReset)}`);
    }

    const resetAck = unityArena.waitForRuntime('intro-reset', null, 3500);
    await unityArena.send('ResetMatchIntro', 'MATCH_START_GATE_LIVE_READY_V1_1');
    await resetAck;

    const afterReset = await getUnityProductionDiagnostics('afterReset');
    afterReset.frontendReadyCache = Boolean(unityArena?.isReady?.());
    if (!afterReset.ready) {
      throw new Error(`Reset 後 frontend ready cache=${afterReset.frontendReadyCache}; Unity LIVE runtime 變成 not-ready：${compactIntroDiag(afterReset)}`);
    }

    const dispatch = waitForIntroDispatchOutcome(3500);
    await unityArena.playMatchIntro();
    let outcome;
    try {
      outcome = await dispatch;
    } catch (error) {
      const timeoutDiag = await getUnityProductionDiagnostics('dispatchTimeout').catch(() => null);
      throw new Error(`${error.message} | ${compactIntroDiag(timeoutDiag)}`);
    }

    window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__ = {
      contract: 'TAPPIE-CHALLENGE-SHARED-INTRO-LIVE-READINESS-DIAGNOSTIC-V1.1',
    ...
    if (outcome.kind === 'status') {
      const live = await getUnityProductionDiagnostics('statusFailure').catch(() => null);
      throw new Error(`Unity PlayMatchIntro 被 runtime status 擋下：${outcome.message} | ${compactIntroDiag(live)}`);
    }
    if (outcome.kind === 'error') {
      const live = await getUnityProductionDiagnostics('runtimeError').catch(() => null);
      throw new Error(`Unity Runtime error during Intro dispatch：${outcome.message} | ${compactIntroDiag(live)}`);
    }
    if (outcome.kind === 'intro-skipped') {
      const live = await getUnityProductionDiagnostics('introSkipped').catch(() => null);
      throw new Error(`Unity Intro 被略過：${outcome.message || 'unknown'} | ${compactIntroDiag(live)}`);
    }

    const completed = await unityArena.waitForRuntime('intro-complete', null, 20000);
    const completedDiag = await getUnityProductionDiagnostics('completed').catch(() => null);
    window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__.completed = true;
    window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__.completeMessage = String(completed?.message || '');
    window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__.completedDiag = completedDiag;
    return completed;
  }
  ...
  async function tryStartMatch() {
    ...
    setGateStatus('雙方已準備，檢查 Unity live ready 並開始入場…');
    enterChallengeButton.textContent = '開始入場';
    $('recordButton').disabled = true;
    setRecordButton('等待入場運鏡完成');
    $('arenaCaption').textContent = '雙方正在進場';
    await window.TappieChallengeState?.transition?.('MATCH_INTRO');
    ...
    try {
      await playMatchIntroWithLiveReadyProbe();
    } catch (error) {
      console.error('[Challenge Arena] Intro live-readiness diagnostic failed', error, window.__TAPPIE_INTRO_LIVE_READY_DIAGNOSTICS__);
      gate.starting = false;
      state.sessionStarted = false;
      $('readyGate').hidden = false;
      $('readyGate').classList.remove('is-leaving');
      enterChallengeButton.hidden = false;
      enterChallengeButton.disabled = false;
      enterChallengeButton.textContent = '重新檢查並開始入場';
      $('recordButton').disabled = true;
      setRecordButton('入場尚未啟動');
      $('arenaCaption').textContent = 'Intro live readiness 診斷';
      setGateStatus(`Intro 未啟動：${error?.message || error}`, true);
      window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__ = {
        ...(window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__ || {}),
        contract: 'TAPPIE-CHALLENGE-SHARED-INTRO-LIVE-READINESS-DIAGNOSTIC-V1.1',
        failed: true,
        ...
        arena: unityArena?.state?.arenaId || null,
        probes: window.__TAPPIE_INTRO_LIVE_READY_DIAGNOSTICS__ || null
      };
      return;
    }
    ...
    await window.TappieChallengeState?.transition?.('ROUND_INPUT');
    prepareRound();
    ...
    window.__TAPPIE_CHALLENGE_GATE_DIAGNOSTICS__ = {
      ...
      introHandshake: window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__ || null,
      arena: unityArena?.state?.arenaId || null,
  ...
  async function playRoundAnimations(type) {
    await window.TappieChallengeState?.transition?.('ROUND_RESULT');
    if (unityArena?.isReady()) {
      const cue = type === 'win'
      ...
      const cueCompleted = await unityArena.playCueAndWait(
        cue,
        'cue-complete',
        12000,
        0
      ).catch(() => null);
      // ROUND_WIN/TIE already owns its accepted return-to-BATTLE_MAIN transition.
      // Only use ResetRoundPose as timeout recovery; never issue a second normal return.
      if (!cueCompleted) await unityArena.resetRoundPose().catch(() => {});
      return;
    }
    if (type === 'win') {
      setFocus('player');
  ...
  async function enterRewardMode() {
    ...
    $('rewardReturnButton').hidden = false;
    $('rewardReturnButton').disabled = false;
    ...
    await window.TappieChallengeState?.transition?.('REWARD_GAMEPLAY');
    const rewardRuntimeArenaId = unityArena?.state?.runtimeProfileId || unityArena?.state?.arenaId;
    await unityArena?.beginRewardSelection({
      arenaId: rewardRuntimeArenaId,
      publicArenaId: unityArena?.state?.arenaId || rewardRuntimeArenaId,
      chestCount: 3,
  ...
  async function finishMatchIfNeeded() {
    ...
    const willEnterReward = playerWon && rewardZoneEnabled && unityArena?.isReady();
    if (unityArena?.isReady()) {
      const finalCueCompleted = await unityArena.playCueAndWait(
        { cue: 'FINAL_WIN', actor: playerWon ? 'player' : 'opponent', returnToBattle: false },
        'cue-complete',
        16000,
        0
      ).catch(() => null);
      // A normal winner Reward handoff preserves the accepted FINAL camera as the
      // start of the adapter-owned blend into the canonical PC2 follow pose.
      // ResetRoundPose remains timeout/non-Reward recovery only.
      if (!willEnterReward || !finalCueCompleted) {
  ...
  async function resolveScoredRound(myScore) {
    ...
    state.round += 1;
    // The accepted Unity cue has already completed its single return to BATTLE_MAIN.
    // Do not enqueue a second ResetRoundPose/camera transition here.
    prepareRound();
  ...
  const releaseRewardLook = event => {
    ...
    void unityArena?.endRewardLook?.().catch?.(() => {});
  };
  ...
  window.addEventListener('tappie:reward-state', event => {
    ...
    if (arenaDebugEnabled) {
      ...
      if (debug) {
        ...
        debug.textContent = `${base}\nreward: ${detail.owner || '-'} · ${detail.state || '-'} · sockets=${detail.socketContract || '-'}\nchests=${detail.chestCount ?? 0} near=${detail.nearChest ? (detail.chestIndex ?? -1) : -1} distance=${fixed(detail.distance)} selected=${detail.selected ? 1 : 0} error=${detail.error || ''}`;
        debug.hidden = false;
      }
    }
    if (!state.rewardMode) return;
    if (detail.selected) {
      ...
      void window.TappieChallengeState?.transition?.('REWARD_SELECTED');
      releaseRewardJoystick();
      rewardChestAction.hidden = true;
      $('rewardReturnButton').hidden = true;
  ...
  $('rewardReturnButton')?.addEventListener('click', async () => {
    if (!state.rewardMode || state.rewardSelected) return;
    const button = $('rewardReturnButton');
    button.disabled = true;
    $('rewardControlStatus').textContent = '正在回到剛剛的對戰位置…';
    try {
      await unityArena?.returnRewardToBattlePose();
      $('rewardControlStatus').textContent = '已回到剛剛的對戰位置，可以重新走向寶箱。';
    } catch (error) {
      console.warn('[Challenge Reward] return-to-battle-pose', error);
      $('rewardControlStatus').textContent = '暫時無法回到對戰位置，請繼續使用搖桿。';
    } finally {
      window.setTimeout(() => { if (!state.rewardSelected) button.disabled = false; }, 420);
    }
  });
  ...
  $('finishRewardMode')?.addEventListener('click', () => {
    void window.TappieChallengeState?.transition?.('EXIT');
