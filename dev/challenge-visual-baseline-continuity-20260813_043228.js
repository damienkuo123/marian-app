(() => {
  ...
  async function getUnityProductionDiagnostics(label, timeoutMs = 3500) {
    ...
    await unityArena.send('GetDiagnostics', label || 'GROUND_DATUM_INTRO_PROBE');
  ...
  function waitForIntroStartOutcome(timeoutMs = 3500) {
    return new Promise((resolve, reject) => {
      const accepted = new Set(['intro-start', 'intro-skipped']);
      const timer = setTimeout(() => {
        window.removeEventListener('tappie:challenge-runtime', listener);
        reject(new Error('Unity Intro 3.5 秒內沒有回報 intro-start / intro-skipped'));
      }, timeoutMs);
      const listener = event => {
        const detail = event.detail;
        const kind = String(detail?.kind || detail?.type || detail?.event || '').toLowerCase();
        if (!accepted.has(kind)) return;
        clearTimeout(timer);
        window.removeEventListener('tappie:challenge-runtime', listener);
        resolve({ kind, detail });
      };
  ...
  async function playMatchIntroGroundDatumProof() {
    ...
    if (!beforeReset.ready) throw new Error(`Unity LIVE runtime 在 Intro 前不是 ready：${compactIntroDiag(beforeReset)}`);

    const resetAck = unityArena.waitForRuntime('intro-reset', null, 3500);
    await unityArena.send('ResetMatchIntro', 'GROUND_DATUM_STABLE_SUPPORT_FIX_PROOF');
    await resetAck;
    const afterReset = await getUnityProductionDiagnostics('afterReset');
    if (!afterReset.ready) throw new Error(`Reset 後 Unity LIVE runtime 不是 ready：${compactIntroDiag(afterReset)}`);

    const startOutcome = waitForIntroStartOutcome(3500);
    await unityArena.playMatchIntro();
    const outcome = await startOutcome;
    window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__ = {
      contract: 'TAPPIE-SELECTION-FACTORY-GROUND-DATUM-STABLE-SUPPORT-PROOF-V1.0',
      outcome: outcome.kind,
      message: String(outcome.detail?.message || ''),
      beforeReset,
      afterReset,
      arena: unityArena?.state?.arenaId || null
    };
    if (outcome.kind === 'intro-skipped') throw new Error(`Unity Intro 被略過：${String(outcome.detail?.message || 'unknown')}`);
  ...
  async function tryStartMatch() {
    ...
    setGateStatus('雙方已準備，驗證 Shared runtime 並開始入場…');
    ...
    try {
      await playMatchIntroGroundDatumProof();
    } catch (error) {
      console.error('[Challenge Arena] ground-datum Intro proof failed', error, window.__TAPPIE_INTRO_LIVE_READY_DIAGNOSTICS__);
      gate.starting = false;
      ...
      enterChallengeButton.hidden = false;
      enterChallengeButton.disabled = false;
      enterChallengeButton.textContent = '重新驗證並開始入場';
      $('recordButton').disabled = true;
      setRecordButton('入場尚未啟動');
      $('arenaCaption').textContent = 'Shared Intro 驗證';
