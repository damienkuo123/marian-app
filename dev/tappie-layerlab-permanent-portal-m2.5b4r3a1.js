(() => {
  ...
  const VERSION = 'M2.5B4R3A1-PRODUCTION-R2';
  const RUNTIME_URL =
  ...
  function rectForTarget(target) {
    ...
    // Keep Unity rendering against the complete wardrobe stage. The outer
    // portal clips at the drawer top, so the drawer covers the avatar instead
    // of forcing the avatar to become progressively smaller.
    if (target.wardrobe) {
      ...
      if (catalog) {
        ...
        if (c.top > r.top && c.top < visibleBottom) {
          visibleBottom = c.top;
        }
      }
    }

    const visibleHeight = Math.max(0, visibleBottom - r.top);
    const renderHeight = target.wardrobe
      ? Math.max(0, r.height)
      : visibleHeight;
  ...
  console.info(
    '[Tappie LayerLab M2.5B4R3A1 Production] permanent portal installed',
    RUNTIME_URL
  );
})();
