(() => {
  ...
  const VERSION = 'M2.5B4R3A5.4-PERSISTENT-SELECTOR-DOM-EXPAND';
  ...
  const EXPAND_ID = 'd32-selector-expand';
  ...
  const model = {
    ...
    gridPanels: new Map(),
    gridPanelUse: new Map(),
    panelSwitchSequence: 0,
    renderSignature: '',
  ...
  async function refreshRemoteThumbnailCatalog() {
    ...
    model.thumbnailRefreshPromise = (async () => {
      ...
      try {
        ...
        model.thumbnailCatalogError = null;
        // Keep existing selector DOM intact. Newly opened categories use
        // refreshed URLs and labels; visible cards are never destroyed.
        if (model.open) syncAllCachedSelectionUi();
        return normalized;
      } catch (error) {
  ...
  function setBusy(value, message = '正在更換…') {
    ...
    // Do not toggle the disabled state of selector cards or tabs. On iOS
    // Safari, disabling a grid full of image buttons can invalidate their
    // composited image layers for a frame, producing the white flash seen
    // during Random and part mutations. Interaction guards remain in the
    // command handlers, while the DOM and image layers stay untouched.
    ...
    if (random) {
      ...
      random.setAttribute('aria-disabled', String(model.busy));
      random.classList.toggle('is-busy', model.busy);
    }

    const tabs = $(TABS_ID);
    if (tabs) {
      tabs.dataset.busy = String(model.busy);
    ...
    const grid = $(GRID_ID);
    if (grid) {
      grid.dataset.busy = String(model.busy);
  ...
  function animationCategoryForKey(key) {
    const active = text(key);
    if (!active.startsWith(ANIMATION_TAB_PREFIX)) return null;
    const name = active.slice(ANIMATION_TAB_PREFIX.length);
    return OFFICIAL_ANIMATION_CATALOG.find(
      entry => entry.category === name
  ...
  function currentRenderSignature() {
    const partTypes = (model.catalog?.parts || [])
      .map(officialPartType)
      .filter(Boolean);
    const animations = OFFICIAL_ANIMATION_CATALOG.map(
      entry => `${entry.category}:${entry.animations.join(',')}`
    );
    return JSON.stringify([partTypes, animations]);
  }

  function resetGridCache() {
    model.panelSwitchSequence += 1;
    model.gridPanels.clear();
    model.gridPanelUse.clear();
    const grid = $(GRID_ID);
    if (grid) {
      grid.innerHTML = '';
      delete grid.dataset.cacheInitialized;
      delete grid.dataset.activeGridKey;
    }
  }

  function prepareGridHost() {
    const grid = $(GRID_ID);
    if (!grid) return null;
    grid.className = 'd32-grid d32-grid-cache-host';
    if (grid.dataset.cacheInitialized !== 'true') {
      grid.innerHTML = '';
      grid.dataset.cacheInitialized = 'true';
    }
    return grid;
  }
  ...
  function syncTabSelectionUi() {
    const host = $(TABS_ID);
    if (!host) return;
    host.querySelectorAll('[data-layerlab-tab-key]').forEach(button => {
      const active = button.dataset.layerlabTabKey === model.activeType;
      button.classList.toggle('active', active);
      button.setAttribute('aria-selected', String(active));
    });
  }

  async function prepareGridPanel(panel) {
    if (!panel || panel.dataset.imagesReady === 'true') return;
    if (panel.__layerlabPreparePromise) {
      await panel.__layerlabPreparePromise;
      return;
    }

    panel.__layerlabPreparePromise = (async () => {
      const images = [...panel.querySelectorAll('img')];
      if (!images.length) {
        panel.dataset.imagesReady = 'true';
        return;
      }
      ...
      // Keep the current panel visible while the next panel prepares. A
      // bounded wait prevents a slow or missing image from blocking tabs.
      await Promise.race([
    ...
    try {
      ...
      panel.__layerlabPreparePromise = null;
    }
  }

  function touchGridPanel(key) {
    model.gridPanelUse.set(key, Date.now());
  }

  function evictOldGridPanels() {
    const maximum = 4;
    if (model.gridPanels.size <= maximum) return;

    const removable = [...model.gridPanels.keys()]
      .filter(key => key !== model.activeType)
      .sort((a, b) =>
        (model.gridPanelUse.get(a) || 0) -
        (model.gridPanelUse.get(b) || 0)
      );

    while (model.gridPanels.size > maximum && removable.length) {
      const key = removable.shift();
      const panel = model.gridPanels.get(key);
      panel?.remove();
      model.gridPanels.delete(key);
      model.gridPanelUse.delete(key);
    }
  }

  function createPanel(key, extraClass = '') {
    const panel = document.createElement('section');
    panel.className =
      `d32-grid-panel d32-layerlab-official-grid ${extraClass}`.trim();
    panel.dataset.gridKey = key;
    panel.hidden = true;
    return panel;
  }

  function buildAnimationPanel(categoryEntry) {
    const key = animationTabKey(categoryEntry.category);
    const panel = createPanel(key, 'd32-animation-grid');
    ...
    for (const animationName of categoryEntry.animations) {
      ...
      button.dataset.internalKey = `animation:${animationName}`;
      button.setAttribute(
        'aria-pressed',
  ...
  function buildPartPanel(part) {
    ...
    for (const { index, item } of options) {
      ...
      button.dataset.internalKey = `${type}:${index}`;
  ...
  function ensureGridPanel(key) {
    const normalizedKey = text(key);
    if (!normalizedKey) return null;
    if (model.gridPanels.has(normalizedKey)) {
      touchGridPanel(normalizedKey);
      return model.gridPanels.get(normalizedKey);
    }

    const host = prepareGridHost();
    if (!host) return null;

    const animationCategory = animationCategoryForKey(normalizedKey);
    let panel = null;
    if (animationCategory) {
      panel = buildAnimationPanel(animationCategory);
    } else {
      const part = catalogPart(normalizedKey);
      if (part) panel = buildPartPanel(part);
    }

    if (!panel) {
      panel = createPanel(normalizedKey, 'd32-official-message-grid');
    ...
    model.gridPanels.set(normalizedKey, panel);
    touchGridPanel(normalizedKey);
    return panel;
  }

  function activateGridPanel(key) {
    const host = prepareGridHost();
    if (!host) return;
    host.querySelectorAll('.d32-grid-panel[data-grid-key]').forEach(panel => {
      panel.hidden = panel.dataset.gridKey !== key;
    });
    host.dataset.activeGridKey = key;
    touchGridPanel(key);
    evictOldGridPanels();
  }

  async function switchActiveType(nextType) {
    const key = text(nextType);
    if (!key || key === model.activeType) return;

    const sequence = ++model.panelSwitchSequence;
    const targetButton = $(TABS_ID)?.querySelector(
      `[data-layerlab-tab-key="${CSS.escape(key)}"]`
    );
    targetButton?.classList.add('is-preparing');

    const panel = ensureGridPanel(key);
    await prepareGridPanel(panel);

    targetButton?.classList.remove('is-preparing');
    if (sequence !== model.panelSwitchSequence) return;

    model.activeType = key;
    syncTabSelectionUi();
    activateGridPanel(key);
  ...
  function renderTabs() {
    ...
    if (host.dataset.renderSignature !== signature) {
      ...
      for (const part of model.catalog?.parts || []) {
        ...
        const button = document.createElement('button');
        button.type = 'button';
        button.className = 'd32-tab';
        button.dataset.layerlabType = type;
        button.dataset.layerlabTabKey = type;
        button.textContent = officialPartLabel(part);
        button.title = type;
        button.addEventListener('click', () => {
          void switchActiveType(type);
        });
  ...
  function ensureSelectorExpandButton() {
    let button = $(EXPAND_ID);
    if (button) return button;

    const wrap = document.querySelector(
      '#dressing-room-overlay .d32-random-cta-wrap'
    );
    if (!wrap) return null;
    ...
    button.id = EXPAND_ID;
    button.type = 'button';
    button.className = 'd32-selector-expand';
    button.setAttribute('aria-label', '展開選項');
    button.setAttribute('aria-expanded', 'false');
    button.innerHTML =
      '<span class="d32-selector-expand-icon" aria-hidden="true">↑</span>';
    wrap.prepend(button);
    return button;
  }

  function applySelectorExpanded(expanded, options = {}) {
    const overlay = $(OVERLAY_ID);
    const button = ensureSelectorExpandButton();
    const next = !!expanded;
    model.selectorExpanded = next;
    overlay?.classList.toggle('is-selector-expanded', next);
    ...
    if (button) {
      ...
      button.title = next ? '收回選項' : '展開選項';
      const icon = button.querySelector('.d32-selector-expand-icon');
    ...
    setTimeout(() => portal()?.refreshLayout?.(), 70);
    setTimeout(() => portal()?.refreshLayout?.(), 240);
  }

  let selectorExpandBound = false;

  function bindSelectorExpand() {
    const button = ensureSelectorExpandButton();
    if (!button || selectorExpandBound) return;
    selectorExpandBound = true;
    button.addEventListener('click', () => {
      applySelectorExpanded(!model.selectorExpanded, {
        keepScroll: true
      });
    });
    applySelectorExpanded(model.selectorExpanded, {
  ...
  function renderGrid() {
    const panel = ensureGridPanel(model.activeType);
    activateGridPanel(model.activeType);
    void prepareGridPanel(panel);
  }
  ...
  function syncAllCachedSelectionUi() {
    for (const type of model.workingSelections.keys()) {
  ...
  function render() {
    const signature = currentRenderSignature();
    if (model.renderSignature && model.renderSignature !== signature) {
      resetGridCache();
    }
    model.renderSignature = signature;
    renderTabs();
    renderToolbar();
    renderGrid();
    syncAllCachedSelectionUi();
  }
  ...
  function showOverlay() {
    ...
    applySelectorExpanded(false, { keepScroll: true });
  ...
  async function open() {
    ...
    try {
      ...
      setDirty(false);
      // First open must build the official tabs and the initial selector
      // panel. Later part changes and Random only sync selected states.
      render();
      window.dispatchEvent(
