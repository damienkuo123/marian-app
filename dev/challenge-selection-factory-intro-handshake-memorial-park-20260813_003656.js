(() => {
  ...
  async function playMatchIntroStrict() {
    if (!unityArena?.isReady()) throw new Error('Unity Arena 尚未 ready，禁止靜默略過 Intro');

    // Frontend owns WHEN the accepted immutable Intro begins. Resetting only the
    // replay state removes stale introPlayed/preview state; it does not alter the
    // accepted Intro profile, vectors, camera path, timing or animations.
    const resetAck = unityArena.waitForRuntime('intro-reset', null, 3500);
    await unityArena.send('ResetMatchIntro', 'MATCH_START_GATE');
    await resetAck;
    ...
    window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__ = {
      contract: 'TAPPIE-CHALLENGE-SHARED-INTRO-HANDSHAKE-V1.0',
    ...
    if (outcome.kind === 'intro-skipped') {
      throw new Error(`Unity Intro 被略過：${String(outcome.detail?.message || 'unknown')}`);
    }
  ...
  async function tryStartMatch() {
    ...
    // The gate must leave before the immutable Intro begins, otherwise the
    // complete Intro can run behind the waiting page.
    $('readyGate').classList.add('is-leaving');
    await new Promise(resolve => setTimeout(resolve, 440));
    $('readyGate').hidden = true;

    try {
      await playMatchIntroStrict();
    } catch (error) {
      console.error('[Challenge Arena] strict intro handshake failed', error);
      gate.starting = false;
      ...
      enterChallengeButton.hidden = false;
      enterChallengeButton.disabled = false;
      enterChallengeButton.textContent = '重新開始入場';
      $('recordButton').disabled = true;
      setRecordButton('入場運鏡未完成');
      $('arenaCaption').textContent = '入場運鏡等待重試';
      setGateStatus(`入場運鏡沒有完成：${error?.message || error}`, true);
      window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__ = {
        ...(window.__TAPPIE_INTRO_HANDSHAKE_DIAGNOSTICS__ || {}),
        contract: 'TAPPIE-CHALLENGE-SHARED-INTRO-HANDSHAKE-V1.0',
        failed: true,
