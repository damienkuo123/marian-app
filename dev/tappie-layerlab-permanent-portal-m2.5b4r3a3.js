(() => {
  ...
  const VERSION = 'M2.5B4R3A3-PRODUCTION-R2';
  const RUNTIME_URL =
  ...
  function rectForTarget(target) {
    ...
    // The wardrobe is now one vertically scrollable page. The permanent
    // runtime iframe stays body-owned, so project only the visible part of
    // the moving stage and offset the iframe by the amount scrolled behind
    // the sticky wardrobe header.
    const overlay = document.getElementById('dressing-room-overlay');
    const topbar = overlay?.querySelector('.d32-topbar');
    ...
    const clipTop = Math.max(
      ...
      topbarRect?.bottom ?? r.top
    );
    const clipBottom = Math.min(
      r.bottom,
      overlayRect?.bottom ?? r.bottom,
      window.innerHeight
    );
  ...
  function sameRect(a, b) {
    ...
    return Math.abs(a.left - b.left) < 0.5 &&
      ...
      Math.abs((a.frameOffsetTop || 0) -
        (b.frameOffsetTop || 0)) < 0.5;
  }

  function placePortal(continuousSync = false) {
    if (!continuousSync) state.layoutRaf = 0;
    ...
    if (!target) {
      stopWardrobeScrollSync();
      portal.classList.remove('is-visible');
      portal.dataset.target = '';
      state.target = null;
      state.visible = false;
      if (!continuousSync) renderDiagnostic();
      return;
    }
    ...
    if (rect.width < 2 || rect.height < 2) {
      if (target.wardrobe) ensureWardrobeScrollSync();
      else stopWardrobeScrollSync();
      portal.classList.remove('is-visible');
      state.visible = false;
    ...
    if (!sameRect(state.lastRect, rect) || state.target !== target.id) {
      ...
      if (runtimeMotion) {
        ...
        runtimeMotion.style.transform =
          `translate3d(0, ${rect.frameOffsetTop || 0}px, 0)`;
      }
      ...
      portal.dataset.frameOffsetTop =
        String(rect.frameOffsetTop || 0);
      state.lastRect = rect;
      state.target = target.id;
    }
  ...
  function wardrobeScrollSyncFrame() {
    if (!isWardrobeOpen()) {
      state.wardrobeSyncRaf = 0;
      return;
    }

    // iOS Safari scrolls the page on a compositor thread. Re-project the
    // body-owned permanent Runtime every frame while wardrobe is open so the
    // character and native page never visibly separate during fast flicks.
    placePortal(true);
    state.wardrobeSyncRaf =
      requestAnimationFrame(wardrobeScrollSyncFrame);
  }

  function ensureWardrobeScrollSync() {
    if (state.wardrobeSyncRaf) return;
    state.wardrobeSyncRaf =
      requestAnimationFrame(wardrobeScrollSyncFrame);
  }

  function stopWardrobeScrollSync() {
    if (!state.wardrobeSyncRaf) return;
    cancelAnimationFrame(state.wardrobeSyncRaf);
    state.wardrobeSyncRaf = 0;
  }

  function schedulePlace() {
    if (state.layoutRaf) return;
    state.layoutRaf = requestAnimationFrame(() => placePortal(false));
  }
  ...
  function bindGeometry() {
    ...
    document.querySelector(
      '#dressing-room-overlay .dressing-room-page'
    )?.addEventListener(
      'scroll',
      schedulePlace,
      { passive: true }
  ...
  console.info(
    '[Tappie LayerLab M2.5B4R3A3 Production] permanent portal installed',
    RUNTIME_URL
  );
})();
