(() => {
  ...
  <b>R3B.3.3.1 Browser Final</b><div id="gate" class="wait">UNITY: waiting for live render…</div><div id="rows"></div><div class="arena">${PAIRS.map(([a,p])=>`<button data-arena="${a}" data-profile="${p}">${a}</button>`).join('')}</div><pre id="out"></pre><div class="hint">Operator checks: Round→BATTLE_MAIN once · Final→Reward 0.72s blend · Walk away → Reward「回到寶箱」must teleport to captured Battle pose + grounded + PC2 behind. ONE active WebGL tab only.</div>`;
