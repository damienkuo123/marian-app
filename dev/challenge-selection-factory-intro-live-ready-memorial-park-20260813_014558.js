(() => {
  ...
  function compactIntroDiag(d) {
    ...
    return `ready=${d.ready} boot=${d.bootStage || '-'} error=${d.lastErrorCode || '-'} player=${d.playerReady}/${d.playerRenderers} opponent=${d.opponentReady}/${d.opponentRenderers} intro=${d.introState || '-'} camera=${d.cameraState || '-'}`;
  }
  ...
  async function playMatchIntroWithLiveReadyProbe() {
    if (!unityArena?.isReady()) throw new Error('Frontend Unity Arena ready cache=false');

    const beforeReset = await getUnityProductionDiagnostics('beforeReset');
    if (!beforeReset.ready) {
      throw new Error(`Unity LIVE runtime 在 Intro 前不是 ready：${compactIntroDiag(beforeReset)}`);
    }
    ...
    if (!afterReset.ready) {
      throw new Error(`Reset 後 Unity LIVE runtime 變成 not-ready：${compactIntroDiag(afterReset)}`);
    }
