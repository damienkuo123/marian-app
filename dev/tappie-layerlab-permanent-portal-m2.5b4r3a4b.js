(() => {
  ...
  const VERSION = 'M2.5B4R3A4B-PRODUCTION-R2';
  const RUNTIME_URL =
    'https://assets.tappieapp.com/avatars/layerlab/casual/2026.07.30-m2.5b4r3a4b/index.html';
  ...
  function rectForTarget(target) {
    ...
    const fullHeight = Math.max(0, r.height);

    // M2.5B4R3A4A2:
    // The white catalog intentionally overlaps the lower part of the stage.
    // Clip the body-owned WebGL portal at the catalog's real top edge so the
    // iframe can never intercept Random, category tabs, or Selector cards.
    let visibleHeight = fullHeight;
    ...
    if (target.wardrobe) {
      ...
      if (catalogRect) {
        visibleHeight = Math.max(
          0,
          Math.min(r.bottom, catalogRect.top) - r.top
        );
      }
    }
  ...
  function placePortal() {
    ...
    if (!sameRect(state.lastRect, rect) || state.target !== target.id) {
      ...
      portal.dataset.interactionClipped =
        target.wardrobe && rect.height < rect.renderHeight ? 'true' : 'false';
  ...
  console.info(
    '[Tappie LayerLab M2.5B4R3A4B Production] permanent portal installed',
    RUNTIME_URL
  );
})();
